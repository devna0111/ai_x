...
SELECT 
    ...
    NAME,
    USERID, 
    TO_CHAR(SALARY, '9,999,999.00') -- 숫자패턴 : 0 또는 9, 빈 공간이 발생하면 (EX 값이 1000인데 옆의 값을 이용하면) 그만큼 공백이 생성된다
        AS SALARY_FORMATTED
FROM MY_DATA;
...
DROP TABLE EMP01;
--1.
CREATE TABLE EMP01 AS SELECT * FROM EMP; -- 테이블 내용 복제, 제약조건은 복제 안됨.
...
SELECT S.SNO 학번, S.SNAME 이름, S.SSCORE 점수, S.MCODE 학과코드, M.MNAME 학과명, M.MOFFICE 학과사무실 
